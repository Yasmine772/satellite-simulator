...
export default class Application {
  ...
  public readonly camera = new THREE.PerspectiveCamera(
    ...
    0.1,
    EARTH_DISTANCE * 1.2 * SIMULATION_SCALE
  );
  public readonly controls = new OrbitControls(
    this.camera,
  ...
  set followedObject(object) {
    ...
    this.camera.position
      .normalize()
      .multiplyScalar(object ? 1e7 : EARTH_RADIUS * 4);
  }
  ...
  constructor(public readonly container: HTMLElement) {
    ...
    this.world.onSatelliteDestruction = (satellite) =>
      this.toaster.toast(
        `${satellite.name} has collided and was destroyed!`,
        "explosion"
      );
    this.showStats = true;
  ...
  private updateInfoPanel() {
    ...
    if (this.world.satellites.length > 0) {
      this.followedObject = this.world.satellites[0];
    }

    if (this.followedObject && this.followedObject instanceof Satellite) {
      const sat = this.followedObject as Satellite;

      this.infoPanel.innerHTML = sat.getOrbitInfo();
    } else {
      this.infoPanel.textContent = `Total Satellites: ${this.world.satellites.length}`;
    }
  }
  ...
  protected followObject() {
    ...
    const cameraDistance = Math.min(
      Math.max(
        this.camera.position.distanceTo(this.followedObject.position),
        0.75e7
      ),
      2e7
    );
    ...
    this.camera.position
      .copy(this.followedObject.position)
      .multiplyScalar((cameraDistance + objectDistance) / objectDistance);
